...
def my_filter_spec_by_scan_id(spec_f, filter_scan_id_set):
    return [spec for spec in spec_f if int(spec.metadata["scans"]) in filter_scan_id_set]

def combine_score_to_matrix(scores, spec_ids):
    df_scores = pd.DataFrame(scores.scores, index=spec_ids, columns=spec_ids)
    return df_scores

def cal_filter_ms2_cosine(spectrums, df_filter, similarity_measure, output_prefix):
    ...
    score_matrix_df.to_csv(f"{output_prefix}_Adjacency_Matrix.csv")
    print(f"{output_prefix}_Adjacency_Matrix.csv has been saved!")
...
if __name__ == '__main__':
    ...
    cal_filter_ms2_cosine(spectrums, df_filter_all, similarity_measure, output_prefix="F7_ModCos")

    end = time.time()
    print(f"The program takes a total of {end - start:.2f} seconds to execute!")
